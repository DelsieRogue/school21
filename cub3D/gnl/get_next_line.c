...
/*   Updated: 2020/12/21 15:03:53 by drogue           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		check_buffer(char **buf, char **line, char **res)
{
	...
	if (ft_strlen2(*buf) != BUFFER_SIZE && !(pi = ft_strchr2(*buf, '\n')))
	{
		if (!(*line = ft_strjoin2(*line, *buf, line)))
			return (full_free(line, res, buf));
		return (0);
	}
	if ((pi = ft_strchr2(*buf, '\n')))
	{
		*pi = '\0';
		if (!(*res = ft_strdup2(pi + 1, res)))
			return (full_free(line, res, buf));
		if (!(*line = ft_strjoin2(*line, *buf, line)))
			return (full_free(line, res, buf));
		return (1);
...
int		buffer_parse(char **buf, char **line, char **res, int fd)
{
	...
	if (i == 0)
	{
		if (!(*line = ft_strjoin2(*line, "\0", line)))
			return (full_free(line, res, buf));
		return (0);
	}
	if (i == -1)
...
int		check_res(char **line, char **res)
{
	...
	if ((pi = ft_strchr2(*res, '\n')))
	{
		*pi = '\0';
		if (!(*line = ft_strdup2(*res, NULL)))
			return (full_free(line, res, NULL));
		if (!(*res = ft_strdup2(pi + 1, res)))
			return (full_free(line, res, NULL));
		return (1);
	}
	if (!(*line = ft_strdup2(*res, res)))
		return (full_free(line, res, NULL));
